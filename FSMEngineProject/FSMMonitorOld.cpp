...
#include "FSMMonitorOld.h"
	...
	void FSMMonitorBeta::initialize() {
		mWasMoved_ = false;
		mIsConnected_ = true;
		mIsPrimary_ = false;
	...
	//FSMMonitorBeta::FSMMonitorBeta() {
	//	initialize();
	//}

	FSMMonitorBeta::FSMMonitorBeta(GLFWmonitor * handle) {

		initialize();
		if (!handle) {
			//fprintf(ERRLOG, "\nERROR! A FSMMonitorBeta object's constructor was called with nullptr!\n"
    ...
    FSMMonitorBeta::~FSMMonitorBeta() noexcept = default;

	FSMMonitorBeta::FSMMonitorBeta(FSMMonitorBeta&& that) noexcept {
	...
	FSMMonitorBeta& FSMMonitorBeta::operator=(FSMMonitorBeta&& that) noexcept {
		if (this != &that) {
			mHandle_ = that.mHandle_;
			mIsConnected_ = that.mIsConnected_;
			mIsPrimary_ = that.mIsPrimary_;
	...
	VideoMode FSMMonitorBeta::getSpecificVideoMode(size_t index) const {
	...
	std::vector<VideoMode> FSMMonitorBeta::getAvailableVideoModes() const {
		return mVideoModes_;
	}

	void FSMMonitorBeta::setCustomGamma(float gamma) {
	...
	void FSMMonitorBeta::getMonitorName() {
	...
	void FSMMonitorBeta::getVideoModes() {
	...
	void FSMMonitorBeta::getMonitorPhysicalSize(int& w, int& h) {
		glfwGetMonitorPhysicalSize(const_cast<GLFWmonitor*>(mHandle_), &w, &h);
	}

#endif //USE_OLD_FSMMONITOR_CODE_
