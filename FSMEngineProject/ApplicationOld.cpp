...
//       Most of the heavy initialization tasks performed by the ApplicationOld
...
//
//             Second task:
//                         Print out some basic information regarding the ApplicationOld
...
//                         asset directory be guarenteed to exist for lifetime of ApplicationOld.
//
//       and that is pretty much all there is to it 
//                         
ApplicationOld::ApplicationOld(int argc, char ** argv) : mRenderEnvironment_(nullptr),
                                                   ABSOLUTE_FILEPATH_TO_EXECUTABLE(std::filesystem::current_path()) {

    //WAIT! Don't move the 'Initializing ApplicationOld' message up here*!!!          *Times I have tried to
...
ApplicationOld::~ApplicationOld() noexcept {
    LOG(TRACE) << __FUNCTION__;
    el::Loggers::flushAll();
}
...
void ApplicationOld::validateAssetDirectories() const noexcept {
    LOG(TRACE) << __FUNCTION__;
    try {
...
FSMInitConfig ApplicationOld::getInitConfig() const noexcept {
...
void ApplicationOld::logApplicationBuildAndConfigurationDetails() noexcept {
    LOG(TRACE) << __FUNCTION__;

    LOG(INFO) << " ";
    LOG(INFO) << "Reporting Executable's Detail";
    //Here would be a good location to log ApplicationOld version number and other such info
...
bool ApplicationOld::createRenderEnvironment() {
    LOG(TRACE) << __FUNCTION__;
    try {
...
//Utility function for converting from any of several possible formats a valid filesystem path
//could be expressed in (i.e. relative/absolute, starts with slash/starts with period) and turns
//each possiblity into the same output format as an absolute filepath.
std::filesystem::path getAbsolutePath(const std::filesystem::path& basePath,
