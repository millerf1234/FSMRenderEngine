...
//                          sense of the sort of input I could expect to receive from some various
...
#include "GLIncludes.h"  //This should be included before glfw
...
void JoystickStatePrinter::buildFullStateMessage(std::stringstream& stateMsg) {
	...
	if (!mPrintedDisconnectMessage_) {
		...
		if (buttons > 0) {
			...
			stateMsg << "Joystick has no available buttons to query the state of...\n";
		}
